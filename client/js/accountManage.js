//Hàm chuyển tab, lưu tab đang được chọn vào localstorage để không bị reload lại trang mặc định khi thực hiện hành động
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function loadProfile() {
                const res = await fetch(`${CONFIG.API_BASE_URL}/api/auth/get_profile.php`, { headers: { 'Authorization': 'Bearer ' + token } });
            ...
            document.getElementById('profileForm').addEventListener('submit', async (e) => {
                ...
                await fetch(`${CONFIG.API_BASE_URL}/api/auth/update_profile.php`, {
                    method: 'POST',
            ...
            async function loadBorrowHistory() {
                const res = await fetch(`${CONFIG.API_BASE_URL}/api/users/borrowings.php`, { headers: { 'Authorization': 'Bearer ' + token } });
                    ...
                    const row = `<tr>
                        <td>
                            <div style="display:flex; align-items:center; gap:10px;">
                                <img src="${CONFIG.IMG_BASE_URL}/${item.image_url}" style="width:30px; height:45px; object-fit:cover; border-radius:3px;">
                                <div>
                                    <b>${item.title}</b><br>
            ...
            async function loadDonationHistory() {
                const res = await fetch(`${CONFIG.API_BASE_URL}/api/users/donations.php`, { headers: { 'Authorization': 'Bearer ' + token } });
                const list = await res.json();
        ...
        async function returnBook(borrowId) {
            ...
            try {
                const res = await fetch(`${CONFIG.API_BASE_URL}/api/users/return.php`, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token},

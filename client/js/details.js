document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`${CONFIG.API_BASE_URL}/api/books/read_single.php?id=${bookId}`);
...
function renderBookDetails(book) {
    ...
    document.getElementById('book-img').src = `${CONFIG.IMG_BASE_URL}/${book.image_url}` || 'img/default-book.png';
...
async function handleBorrowDetail(bookId) {
    ...
    try {
        const response = await fetch(`${CONFIG.API_BASE_URL}/api/borrowings/create.php`, {
            method: 'POST',
            headers: {
